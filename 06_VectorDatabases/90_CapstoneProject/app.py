...
db = Chroma(persist_directory="db", collection_name="movies", embedding_function=embedding_model)
#%% develop the app
st.title("Filmfinder")
...
user_query = st.chat_input("Worum geht es in dem Film?")
if user_query:
    # Retrieve the most similar movies
    with st.spinner("Suche nach ähnlichen Filmen..."):
    ...
    # Display the results
    st.header(f"Ähnlichste Filme: ")
    st.subheader(f"Frage: '{user_query}'")
    cols = st.columns(4)
